...
**pointfoot-for-highlevel 为用户提供上层应用接口。**
...
- 编译运行
  ...
  ```
  $ mkdir -p pointfoot-for-highlevel/src
  $ git clone https://github.com/limxdynamics/pointfoot-for-highlevel.git pointfoot-for-highlevel/src/pointfoot-for-highlevel
  $ cd pointfoot-for-highlevel
  $ catkin_make install
...
- 配置机器人IP地址
  ...
  打开`pointfoot_highlevel.launch`设置`robot_ip`参数的值为机器人IP地址。如下所示：

  ```
  <launch>
    <param name="robot_ip" value="192.168.1.2" />
    <node name="pointfoot_highlevel_node" pkg="pointfoot_highlevel" type="pointfoot_highlevel_node" output="screen" required="true"/>
  </launch>
  ```
...
- 接口介绍
  ...
  | 主题                   | 类型                             | 功能                         |
  | ---------------------- | -------------------------------- | ---------------------------- |
  | /topic/limx/twist      | geometry_msgs/Twist              | 接收控制机器人行走的控制命令 |
