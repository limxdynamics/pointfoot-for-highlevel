...
int main(int argc, char** argv)
{
  ...
  // Create publishers and subscriber for IMU data and diagnostic status messages
  ros::NodeHandle nh;
  ...
  // Retrieve the robot IP parameter from the NodeHandle, defaulting to "192.168.1.2" if not found
  std::string ip = nh.param<std::string>("robot_ip", "192.168.1.2");
  ...
  if (robot->init(ip)) {
    ...
    robot->subscribeImuData([&](const limxsdk::ImuDataConstPtr& imu_data) {
      ...
      imu_msg.angular_velocity.x = imu_data->imu_gyro[0];
      imu_msg.angular_velocity.y = imu_data->imu_gyro[1];
      imu_msg.angular_velocity.z = imu_data->imu_gyro[2];
      imu_msg.linear_acceleration.x = imu_data->imu_acc[0];
      imu_msg.linear_acceleration.y = imu_data->imu_acc[1];
      imu_msg.linear_acceleration.z = imu_data->imu_acc[2];
      imu_msg.orientation.w = imu_data->imu_quat[0];
      imu_msg.orientation.x = imu_data->imu_quat[1];
      imu_msg.orientation.y = imu_data->imu_quat[2];
      imu_msg.orientation.z = imu_data->imu_quat[3];
